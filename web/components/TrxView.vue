<template>
    <div class='item'>
        <div class='blockspec'>
            <span><i>TX#:</i> <nuxt-link :to="$i18n.path('/trx/' + item.trx_id)">{{ "0x" + item.trx_id.toLocaleUpperCase() }}</nuxt-link></span>
            <span><i>Payer:</i> <span class="shortspan"><span :title="item.payer.toLocaleUpperCase()">{{ item.payer.toLocaleUpperCase() }}</span></span>
                  &nbsp;&nbsp;on&nbsp;&nbsp;&nbsp; <nuxt-link :to="$i18n.path('/block/' + item.block_id)"><span class="shortspan"><span :title="'0x' + item.block_id.toLocaleUpperCase()">{{ "0x" + item.block_id.toLocaleUpperCase() }}</span></span></nuxt-link></span>
